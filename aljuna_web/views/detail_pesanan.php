...
$tampil = $pesanan->tampil(@$_GET['id_pesanan']);
...
<div class="row">
  <div class="col-lg-12">
    ...
    <ol class="breadcrumb">
      ...
      <li>
        <a href="?page=pesanan">
          <i class="icon-dashboard"></i> Pesanan</a>
      </li>
      <li class="active">
  ...
  <div class="col-lg-12">
    <div class="table-responsive">
      ...
      <table class="table table-bordered table-hover table-striped" id="datatables">
        ...
        <tbody>
          <?php
          $no = 1;
          $tampil = $pesanan->tampil_detail(@$_GET['id_pesanan']);
    ...
    <div class="col-lg-12">
      ...
      <form id="form" action="" method="post" enctype="multipart/form-data">
        ...
        <input type="submit" class="btn btn-danger" name="batal" value="Batalkan" onclick="return confirm('Batalkan pesanan?')"
          <?php if ($data->status == '4' || $data->status == '5') echo 'Disabled'; ?>>
      </form>
    </div>
    ...
    if (@$_POST['kirim']) {
      ...
      $pesanan->kirim($data->id_pesanan, $resi);
      header("location:?page=detail_pesanan&id_pesanan=" . $data->id_pesanan);
    } else if (@$_POST['selesai']) {
      $pesanan->selesai($data->id_pesanan);
      header("location:?page=detail_pesanan&id_pesanan=" . $data->id_pesanan);
    } else if (@$_POST['batal']) {
      $pesanan->batal($data->id_pesanan);
      header("location:?page=detail_pesanan&id_pesanan=" . $data->id_pesanan);
    } else if (@$_POST['tolak']) {
      $pesanan->tolak($data->id_pesanan);
      header("location:?page=detail_pesanan&id_pesanan=" . $data->id_pesanan);
    }
    ?>